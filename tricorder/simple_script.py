...
for config_fname in ['test7_3d']:
    for sigma_z in [0.001,0.002,0.003,0.004,.005]:
        for min_z in [.15,.3,.45]:
            max_z = min_z + 0.15
            command_str = "import simple; simple.calc_3pt_noisy_photoz(" + str(0) + ", '" + config_fname + "', " + str(min_z) + "," + str(max_z) + "," + str(sigma_z) + "," + "'REDSHIFT','Z')"
            print command_str
            ...
            #for dset_id in range(36):
                command_str = "import simple; simple.calc_3pt_noisy_photoz(" + str(dset_id) + ", '" + config_fname + "', " + str(min_z) + "," + str(max_z) + "," + str(sigma_z) + "," + "'ZSPEC','Z')"
