...
def get_zspec(is11k=False):
    ...
    for i,zmin in enumerate([.15,.3,.45,.6]):
        ...
        cc.configure(legend_kwargs={"loc": "lower right"},label_font_size=18,tick_font_size=14)
...
def get_zrm_only(is11k=False):
    ...
    for i,zmin in enumerate([.15,.3,.45,.6]):
        ...
        plt.suptitle(str(zmin)+r'$ < z < $'+str(zmax),y=1)
        plt.legend()
        plt.savefig('./figures/rmonly_bin'+str(i+1)+str11k+'.pdf',dpi=300,bbox_inches='tight')
        plt.savefig('./figures/rmonly_bin'+str(i+1)+str11k+'.png',dpi=300,bbox_inches='tight')
